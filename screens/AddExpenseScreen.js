...
export default function AddExpenseScreen({ navigation, route }) {
  ...
  const [date, setDate] = useState(editing ? new Date(`${editing.date}T00:00:00`) : new Date());
  ...
  function handleSave() {
    if (!isValid) return;
    const fields = { amount, categoryId, date: toISODate(date), note: note.trim(), photoUri };
    if (editing) {
  ...
  function isDirty() {
    ...
    return amountText.trim() !== '' || categoryId !== null || note.trim() !== '' || photoUri !== null;
  }
      ...
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        ...
        <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.chipGrid}>
            {CATEGORIES.map((c) => (
              <CategoryChip key={c.id} category={c} selected={categoryId === c.id} onPress={setCategoryId} />
            ))}
          </View>

          <Text style={styles.sectionLabel}>Date</Text>
          <Pressable style={styles.fieldRow} onPress={() => setShowPicker(true)} accessibilityRole="button">
        ...
        <View style={styles.footer}>
          ...
          >
            <Text style={styles.saveBtnText}>{editing ? 'Save Changes' : 'Save Expense'}</Text>
          </Pressable>
        </View>
      </KeyboardAvoidingView>
    </SafeAreaView>
...
const styles = StyleSheet.create({
  ...
  currencySymbol: { fontSize: typography.display, fontWeight: typography.bold, color: colors.textSecondary, marginRight: spacing.xs },
  amountInput: { flex: 1, fontSize: typography.display, fontWeight: typography.bold, color: colors.textPrimary, padding: 0 },
  sectionLabel: {
  ...
  chipGrid: { flexDirection: 'row', flexWrap: 'wrap', marginBottom: spacing.lg },
  fieldRow: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'space-between',
  ...
  doneBtnText: { color: colors.primary, fontWeight: typography.semibold, fontSize: typography.body },
  noteInput: {
    backgroundColor: colors.background,
    borderRadius: radii.md,
    paddingHorizontal: spacing.md,
  ...
  photoBtnText: { marginLeft: spacing.xs, color: colors.textSecondary, fontSize: typography.body },
  photoPreviewWrap: {
    borderRadius: radii.md,
    overflow: 'hidden',
    height: 160,
  ...
  saveBtnText: { color: colors.textInverse, fontSize: typography.subtitle, fontWeight: typography.semibold },
});
