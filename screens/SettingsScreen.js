...
import { getExchangeRate, currencyCodeForSymbol } from '../services/exchangeRate';
...
export default function SettingsScreen() {
  const { reminderEnabled, reminderHour, reminderMinute, currencySymbol, updateSettings } = useSettings();
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.card}>
          <View style={styles.row}>
            <View style={styles.rowText}>
              ...
              <Text style={styles.rowCaption}>{formatTime(reminderHour, reminderMinute)}</Text>
            </View>
            <Switch
          ...
          <Pressable style={[styles.row, styles.rowBorder]} onPress={cycleCurrency} accessibilityRole="button">
        ...
        <View style={styles.card}>
          <Pressable style={styles.row} onPress={handleClearData} accessibilityRole="button">
        ...
        <View style={styles.card}>
          <Text style={styles.aboutText}>Expense Tracker · CM3050 Mobile Development final project</Text>
          {deviceId ? (
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: typography.title, fontWeight: typography.bold, color: colors.textPrimary, marginBottom: spacing.lg },
  sectionLabel: {
  ...
  card: { backgroundColor: colors.surface, borderRadius: radii.md, overflow: 'hidden' },
  row: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  rowText: { flex: 1 },
  rowLabel: { fontSize: typography.body, color: colors.textPrimary, fontWeight: typography.medium },
  rowCaption: { fontSize: typography.caption, color: colors.textSecondary, marginTop: 2 },
  ...
  aboutText: { fontSize: typography.body, color: colors.textSecondary, padding: spacing.md },
  deviceIdText: {
    fontSize: typography.caption,
    color: colors.textSecondary,
    paddingHorizontal: spacing.md,
